...
## [You just saw some awesome AI examples from Joe]{.r-fit-text}

![](images/paste-3.png){.rounded-shadow fig-align="center" width="25%"}


## New Tools for LLMs {.smaller}
...
## Let's (almost) Build a Chatbot {auto-animate="true" auto-animate-easing="ease-in-out"}
...
![A very simple chatbot](images/simple-chatbot.png){.rounded-shadow width="50%"}
...
``` {.r code-line-numbers="|8-11|12-15"}
library(shiny)
...
![](images/paste-1.png){.rounded-shadow fig-align="center"}
...
bio_spec <- type_object(
  ...
  favorite_pkg = type_string("Favorite R package")
)
chat$extract_data(bio, spec = bio_spec)
```
...
## [Sidebots - Why are they so useful?]{.r-fit-text style="color: rgb(15, 56, 154); background-color: rgba(255, 255, 255, .7);"} {.center background-image="images/r-sidebot.png"}
