...
![](images/advice-from-joe.png){.rounded-shadow}
...
## Let's Build a Chatbot {auto-animate="true"}

## Let's Build a Chatbot {auto-animate="true" auto-animate-easing="ease-in-out"}
...
::: {.column}
![](images/nejm_screenshot.png){.rounded-shadow}
:::
...
```{.r}
chat <- chat_openai(model = "gpt-4o")                  #<1>
text <- read_pdf(file_path = "NEJMoa2302392.pdf")      #<2>
extracted_data <- chat$extract_data(text, spec = study_info_spec) #<3>
extracted_tbl <- studies_to_table(extracted_data)      #<4>
```

1. Create a chat object
2. Extract text with `read_pdf` helper function
3. Structured output call with custom specifiction
...
## Sidebots - Why are they so useful? {.center}
